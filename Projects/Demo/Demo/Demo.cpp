...
class BitPattern
{
//	BitPattern(short i, char s, int d){a=i; b=s; c=d;}
public: char a:7, b:4, c:3;
};
...
public:
	...
	const int  i;
	Demo():i(123) {O();}
	/*__declspec(dllexport)*/ virtual void O()
	{
...
void ThdFun(void* parm)
{
	cout<<"In individual thread..."<<endl;
...
#if 1
   cout<<"Size of static const member pointer is:"<< sizeof(pf)<<"\nSize of const member pointer is:"<<sizeof(pi)<<"\nSize of member function pointer is:"<<sizeof(pO)<<endl;
#endif

	DWORD dwThreadID;
	hEvt=CreateEvent(0,false,false,"Thread Event");
	HANDLE hThread;
	if(!(hThread=CreateThread(0, NULL, LPTHREAD_START_ROUTINE (ThdFun), 0, NULL, &dwThreadID)))
		cerr<<"Create thread failed..."<<endl;
	cout<<"Create thread succeeded:\t"<<dwThreadID<<endl;
	SetEvent(hEvt);
	...
	cout<<"Finished!\t"<<GetCurrentThreadId()<<endl;
